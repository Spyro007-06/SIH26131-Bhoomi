      ...
      <CardHeader className="pb-3 border-b border-bhoomi-border/60">
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm font-semibold uppercase tracking-wider text-bhoomi-text-secondary flex items-center gap-1.5">
            <HelpCircle className="h-4 w-4 text-bhoomi-green-700" />
        ...
        {observations.length === 0 ? (
          ...
          <div className="space-y-3">
            {observations.map((obs, idx) => (
              <div
                key={idx}
                className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 p-3 rounded-lg border border-bhoomi-border bg-bhoomi-surface-soft/30 text-xs"
              >
                <div className="space-y-1 max-w-xl">
                  <p className="font-medium text-bhoomi-text leading-relaxed">
