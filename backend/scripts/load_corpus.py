...
SOURCES_NEEDED_PATH = CORPUS_DIR / "SOURCES_NEEDED.md"
...
def _write_sources_needed_report(needed: list[SourceNeeded]) -> None:
    lines = [
        "# SOURCES_NEEDED.md",
        "",
        "Generated by scripts/load_corpus.py. A report for a human, not an input",
    ...
    for n in needed:
        lines.append(
            f"| `{n.doc_id}` | `{n.target}` | {n.source_raw or '*(missing)*'} | "
            f"{'; '.join(n.rules)} |"
        )
    lines.append("")
    SOURCES_NEEDED_PATH.write_text("\n".join(lines), encoding="utf-8")
...
async def load(dry_run: bool = False) -> int:
    ...
    sources_needed: list[SourceNeeded] = []

    for index, row in enumerate(raw_rows, start=1):
        chunks, refusal, mismatch = _validate_row(row, index)
        if refusal:
            refusals.append(refusal)
            source_rules = [r for r in refusal.reasons if r.startswith("source")]
            if source_rules:
                sources_needed.append(
                    SourceNeeded(
                        doc_id=refusal.doc_id, target=refusal.target_raw,
                        source_raw=str(row.get("source", "")), rules=source_rules,
                    )
                )
        else:
    ...
    if sources_needed:
        _write_sources_needed_report(sources_needed)
    ...
    if sources_needed:
        print(
            f"\n  {len(sources_needed)} row(s) need a source/source_dated fix, written to "
            f"{SOURCES_NEEDED_PATH.relative_to(CORPUS_DIR.parents[1])}"
        )
